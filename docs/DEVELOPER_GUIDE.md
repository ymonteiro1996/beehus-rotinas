...
│   ├── controle_carteiras.py # Wallet control (templates + detail grids)
│   ├── dashboard.py          # Controle Diário (daily position grid)
│   ├── impostos.py           # Tax view for wallet securities
│   ├── issues.py             # Data quality issues + security wizard
│   ├── nav.py                # NAV package tracking
│   ├── precificacao.py       # Security pricing (pos-fixado, pre-fixado HTM, inflacao HTM)
│   ├── precos.py             # Historical price viewer (multi-security, multi-source)
│   ├── repeticao.py          # Position repetition/cloning tool
│   ├── report_builder.py     # Multi-collection report builder + Excel export
...
│   ├── conciliacao.html
│   ├── conciliacao_transactions.html
│   ├── config.html
│   ├── controle_carteiras.html
│   ├── dashboard.html
│   ├── impostos.html
│   ├── issues.html
│   ├── nav.html
...
├── settings.json             # Global UI toggles and search blacklist
├── user_connections.json     # Per-Windows-user MongoDB connection strings
├── default_blacklist.json    # Default token blacklist for security search wizard
├── saved_reports.json        # User-saved report configurations
├── precificacao_config.json  # Benchmark configuration for precificacao
├── precificacao_lists.json   # Saved security lists for precificacao
├── conciliacao_settings.json # Conciliation engine settings (execPrice security types)
├── wallet_templates.json     # Wallet control templates (groupings + thresholds)
├── repeticao_templates.json  # Position repetition templates
├── report_wallet_templates.json # Report builder wallet templates
├── deriv_mappings.json       # Derivative transaction mappings for report builder
└── requirements.txt
```
...
`company_filter` is an array of `companyId` strings. When non-empty, only those companies are shown on the dashboard and conciliação pages. An empty array means show all companies.
...
`db.py` also exports helpers used by dashboard and nav grids:
...
| `issues` | `_id`, `type`, `companyId`, `entityId`, `walletId`, `date`, `status`, `description` |
...
**`transactions.beehusTransactionType`** can be `null` for unclassified transactions. Always handle the null case. Known types include: `buySell`, `coupon`, `amortization`, `dividend`, `withdrawalDeposit`, `securityTransfer`, `taxes`, `gainsExpenses`, `rebate`.

### Issue types

`missing_wallet`, `missing_unprocessed_position`, `security_unmapped`, and others — see `pages/issues.py` for the full list.
...
| `PRECIFICACAO.md` | Specification of pricing calculation types (pos-fixado, pre-fixado HTM, inflacao HTM) and their formulas. |

...
|-----------|---------|---------|
| `DateCards(container, cards, onSelect)` | dashboard, issues, conciliacao, validacao | Grid of clickable date cards with count |
| `CompanySelector(container, companies, onChange)` | conciliacao, validacao, precificacao, impostos | Dropdown with company filter |
...
| `issues.html` | Use `DateCards`, `DataTable` (already uses server-rendered cards) |
| `precificacao.html` | Use `CompanySelector` |
| `precos.html` | Use `DataTable` |
| `impostos.html` | Use `CompanySelector`, `DataTable` |
| `repeticao.html` | Use `CompanySelector`, `DataTable` |
| `report_builder.html` | Use `CompanySelector` |
...
3. **issues.html** — date cards + table pattern
4. **dashboard (index.html)** — core grid views
5. **precificacao.html** — search + calculation
6. **report_builder.html** — complex but self-contained
7. **repeticao.html** — simple generate flow
8. **precos.html** — search + chart
9. **impostos.html** — simple table view

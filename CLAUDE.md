...
- **Código já existente no projeto ainda não segue esse padrão** (foi escrito antes
  desta convenção). Não é preciso reescrever tudo de uma vez: ao **tocar** numa função
  para corrigir ou evoluir algo, adicione o comentário contexto + pseudocódigo naquele
  momento. Código novo já nasce com o comentário.
...
- Cada tela é um **blueprint** com **responsabilidade única** — o projeto já segue
  essa divisão (`pages/dashboard.py`, `pages/controle_carteiras.py`,
  `pages/controle_demandas.py`, `pages/rotinas_diarias.py`, `pages/rotinas_v2.py`,
  `pages/banda_rentabilidades.py`, `pages/saude_operacional.py`, `pages/setup.py`,
  `pages/nav.py`, `pages/config.py`) — mantenha novas telas no mesmo padrão.
...
- O projeto **ainda não tem** nenhuma tela quebrada nesse padrão de pasta (JS hoje
  vive embutido nos templates) — ao refatorar a primeira tela grande ou criar uma
  tela nova com JS não-trivial, já nasça seguindo essa estrutura.
...
- Crie e use uma pasta **`utils/`** na raiz do projeto para **funções genéricas e
  ...
  negócio de nenhuma página específica. **O projeto ainda não tem essa pasta** — crie-a
  na primeira vez que surgir um helper genérico (formatação, datas, validação) que
  hoje estaria duplicado entre `pages/*.py`.
...
- **Antes de escrever um helper novo, verifique se ele já existe** (em `db.py`, em
  algum `pages/*.py`, ou já em `utils/` se já tiver sido criada). Se algo parecido
  existir, reutilize ou generalize o existente em vez de duplicar.
- Quando notar a **mesma lógica repetida em duas ou mais páginas** (ex.: cálculo de
  dias úteis, hoje duplicado entre `db.py` e páginas — ver `get_biz_dates`/
  `biz_days_elapsed`), extraia para `utils/` e faça as páginas passarem a importar
  de lá.
...
- Antes de implementar, **procure se já existe** função/helper que resolva o problema
  (em `utils/`, no blueprint da tela, ou em módulos como `beehus_api/` — quando
  existir, ver seção 8 —, `db.py`).
...
responsabilidades num único arquivo, criar um novo monólito, ou (ver seção 8) escrever
direto no Mongo quando não há endpoint homologado para aquele dado —, então:
...
O objetivo do projeto é uma camada de dados **API-first, com o Mongo apenas como
fallback de leitura** — mesmo padrão já adotado nos apps-irmãos (`SWAT\ControleCargas`,
`SWAT\Conciliacao`, `SWAT\Controle de cargas`, `SWAT\Transactions`, `SWAT\Tombamento`,
`SWAT\Pricing`), que consomem a API Beehus através de um pacote local `beehus_api/`.

> **[STATUS ATUAL — 2026-08-10] Migração PARCIAL, pausada por decisão do usuário** ("dar
> importância só à Banda de Rentabilidade" → parar a migração geral no que já foi feito).
> `beehus_api/` (cópia do cliente HTTP dos apps-irmãos) já existe no projeto, com o
> token gerenciado em `/api/beehus-token` (masthead, ver `app.py` + `static/js/
> beehus_token.js`). **Já 100% API-first:** `pages/config.py`, `pages/nav.py`,
> `pages/banda_rentabilidades.py`, `pages/dashboard.py`, `pages/rotinas_v2.py` (este
> último não precisou de nenhuma mudança — só usava helpers de `db.py` já migrados).
> **Ainda 100% Mongo direto** (não tocado nesta rodada): `pages/rotinas_diarias.py`
> (1400+ linhas — mistura carteiras de TODAS as empresas num grid do mês inteiro; tem
> o mesmo formato caro que a Banda de Rentabilidade descobriu na prática, ver nota
> abaixo) e `pages/saude_operacional.py` (importa helpers de `rotinas_diarias.py`,
> herda a mesma pendência). `pages/setup.py` continua como está (tela de registro da
> conexão Mongo — sem mudança de escopo). **Todo o acesso Mongo remanescente já é
> 100% leitura** (a única escrita é `ensure_indexes()`, estrutural, não dado). Ver as
> notas específicas abaixo para os gaps confirmados e o custo real medido.

- **Nenhuma tela nova** deve fazer HTTP cru para a API Beehus nem acessar o Mongo sem
  passar por uma camada central (`db.py` e, conforme for migrando, `beehus_api/`).
  Isso evita que cada página reinvente o acesso a dados.
...
- **PROIBIDO** (acesso direto ao banco): `insert_one`/`insert_many`, `update_one`/
  ...
  `find_one_and_*`, ou qualquer comando que **modifique** dados no Mongo. (Criação de
  índice via `create_index` é estrutural, não dado, e já é o único caso hoje.)
...
- **Toda alteração de dados** (criar, editar ou remover) **deve passar exclusivamente
  pelas rotas homologadas da API Beehus**, através dos módulos de `beehus_api/` (ex.:
  `securities`, `transactions`, `positions`, `provisions`) quando esse pacote existir
  neste projeto.
...
> Resumindo: **Mongo = leitura (hoje, tudo; após a migração, só fallback). Escrita =
> só via API homologada.**

---

## 9. Segredos e dados de cliente (segurança)

- **Segredos só via variável de ambiente** (`MONGO_URI`, token da API Beehus, `.env`,
  etc.). **Nunca** deixe credenciais, tokens, URIs ou senhas hardcoded no código, e
  **nunca** as comite (`.env` e `data/user_connections.json` já estão no `.gitignore`).
  Um token JWT colado numa conversa/chat **não é um segredo de código** — nunca o
  escreva em arquivo versionado, `.md` de documentação, ou log.
- **Nunca logue nem exponha credenciais** em mensagens de erro ou telas. Mantenha esse
  cuidado em código novo (a URI do Mongo, por exemplo, nunca deve aparecer em resposta
  de erro ao usuário).
- A pasta `data/` pode conter dados reais de cliente/configuração sensível — confira o
...
- O projeto externaliza regras e parâmetros em JSON (`data/config.json`,
  `data/settings.json`, `data/nav_settings.json`, `data/default_blacklist.json`, etc.).
  **Parâmetros configuráveis vão para `data/*.json`**, não hardcoded no meio da lógica.
- Atenção a quais arquivos são versionados e quais são locais/sensíveis: o `.gitignore`
  faz essa distinção (ex.: `data/user_connections.json`, planilhas `.xlsx`/`.xls`).

---

## 11. Manter a documentação (`docs/`) atualizada

- O projeto tem documentação por tela/fluxo em `docs/` (ex.: `DEVELOPER_GUIDE.md`,
  `PRECIFICACAO.md`, `CONCILIACAO_*.md`, `FILE_GENERATION.md`).
...
- O `requirements.txt` é enxuto. **Não adicione biblioteca nova** (Python ou JS) **sem
  confirmar com o desenvolvedor** — isso conta como quebra de padrão (seção 7). Prefira
  resolver com o que já existe no projeto.
...
- Ao rodar dashboards Python (Dash/Flask), sempre use `use_reloader=False` em
  ...
  caminho com espaços, avise proativamente e configure de acordo. Este projeto já vive
  em `OneDrive - OIkos FAMILY OFFICE\Área de Trabalho\Projeto - Servidor\beehus-rotinas`
  (caminho com espaços e acentos) — `app.py` já usa `use_reloader=False`.
- O ambiente é **Windows** e o conteúdo tem **acentos em português**. **Salve os arquivos
  em UTF-8** e tome cuidado com acentuação no terminal.

---

## Notas específicas deste projeto (beehus-rotinas) — [ADICIONADO 2026-08-10]

- **Escopo:** 10 blueprints em `pages/` (`dashboard`, `controle_carteiras`,
  `controle_demandas`, `rotinas_diarias`, `rotinas_v2`, `banda_rentabilidades`,
  `saude_operacional`, `setup`, `nav`, `config`). `pages/setup.py` continua
  registrando a conexão Mongo por usuário Windows (singleton `db` de `db.py`,
  `_DbProxy`) — necessário porque parte do app ainda lê Mongo direto (ver abaixo).

- **Migração API-first — feita para `config`/`nav`/`banda_rentabilidades`/`dashboard`/
  `rotinas_v2` [2026-08-10, a pedido do usuário: "consegue efetuar todas consultas por
  Endpoints" → depois "dar importância só à Banda de Rentabilidade" → migração geral
  parada aí].** `pages/rotinas_diarias.py` e `pages/saude_operacional.py` (que importa
  helpers de `rotinas_diarias.py`) **continuam 100% Mongo direto, não tocados nesta
  rodada** — ver "Pendência" abaixo antes de migrá-los no futuro.

- **`beehus_api/`** (cópia do cliente HTTP dos apps-irmãos, versão mais completa
  copiada de `SWAT\Conciliacao\beehus_api`) e a camada central em `db.py`:
  - `catalog_companies()`/`catalog_company_names()`, `catalog_entities()`/
    `catalog_entity_names()`, `catalog_wallets(company_ids=...)` + `filter_wallets(...)`
    — substituem `db.companies/entities/wallets.find` (cache 120s). **Atenção de
    segurança:** o doc bruto de `list_companies()` traz `externalAuths[].clientSecret/
    certificate/key` (credenciais de integração, ex. certificado Itaú) — `catalog_companies()`
    já descarta tudo isso e devolve só `_id`/`name`; nunca repasse o retorno cru de
    `list_companies()`/`beehus_api.partner` pra um template ou `jsonify`.
  - `api_unprocessed_positions`/`api_processed_positions[_multi]`/`api_nav_results
    [_multi]`/`api_transactions` — substituem os aggregates de
    `unprocessedSecurityPositions`/`processedPosition`/`navPackages`/`transactions`.
    A API é **sempre por-empresa** (nunca cross-empresa como o Mongo) e a de posição
    processada/NAV só aceita **1 data por chamada** (sem faixa) — por isso essas
    funções fazem fan-out (empresas × datas) com `ThreadPoolExecutor` e cacheiam cada
    combinação individualmente. `normalize_id()`/`_normalize_ids()` cuidam de vários
    endpoints devolverem `walletId`/`entityId` **populados** (objeto) em vez do id cru
    que o Mongo devolvia.
  - `catalog_securities()`/`catalog_securities_by_id()`, `catalog_groupings(company_id)`
    — substituem `db.securities.find`/`db.groupings.find`. **Bug pré-existente
    corrigido de trânsito:** a aba "Agrupamentos" da Banda de Rentabilidade lia
    `db.groups` (collection **sem nenhum documento** — sempre voltava vazia); a
    migração trocou para `catalog_groupings` (endpoint correto, `groupings`).
  - **Cache com TTL diferenciado** (`_ttl_for_date`): 60s pra data de hoje/futura
    (ainda carregando ao longo do dia), **1h pra data passada** (não muda mais) — ver
    "Custo real medido" abaixo pra por quê isso foi necessário.

- **Autenticação da API Beehus:** token Bearer válido por ~1 dia, colado pelo usuário
  no botão "🔑 Beehus API" da masthead (`GET/POST/DELETE /api/beehus-token` em
  `app.py`, modal em `static/js/beehus_token.js`, template em `templates/base.html`).
  Guardado em memória de processo + persistido em `~/.swat/beehus.token` (mesmo
  mecanismo dos apps-irmãos, sobrevive a restart enquanto o token não expirar). O
  `before_request` de `app.py` continua exigindo Mongo registrado (`/setup`) porque
  `rotinas_diarias`/`saude_operacional` ainda dependem dele — as rotas de token e
  `/setup` são as únicas isentas desse gate.

- **Gaps confirmados sem endpoint homologado** (validados também nos apps-irmãos,
  inclusive `SWAT\Issues`, que ainda lê `db.issues` direto):
  1. **Collection `issues`** — nenhum endpoint expõe as issues brutas por
     carteira/tipo/status. Usado hoje em `pages/dashboard.py` (`get_wallet_template_detail`)
     — fica em Mongo, com o comentário `# Gap confirmado [2026-08-10]` no código.
  2. **"Última data com dado, em lote"** (ex.: `db.navPackages.find_one(sort=...)` pra
     achar a data mais recente entre várias carteiras de uma vez) — sem endpoint
     equivalente; usado em `pages/banda_rentabilidades.py` (`get_dates`) — fica em Mongo.
  3. **`published` de `navPackage` POR CARTEIRA** — `get_nav_results`/
     `walletsWithNavDetailed` não tem esse campo (só existe a nível de agrupamento,
     em `groupingsDetailed`). Usado no filtro "Publicado" de
     `pages/banda_rentabilidades.py` (`_published_wallet_ids`, `_fetch_nav_data`) —
     fica em Mongo só quando esse filtro está ativo.
  4. `cashAccounts` — sem uso direto identificado em nenhuma página hoje (só listada
     em `ensure_indexes`); não migrado por não haver nada a migrar.
  Nenhum desses tem workaround: são leitura Mongo, documentada, permitida pela seção 8.

- **Custo real medido (por que o cache de 1h existe):** a Banda de Rentabilidade
  precisa de até ~66-313 dias úteis de histórico (banda EWMA) pra **1 empresa** — como
  a API só aceita 1 data por chamada, a 1ª carga de uma empresa/data nova mediu
  **75-103s** (Ativos 100s, Carteiras 76s, Benchmarks 103s), bem mais caro que o
  fan-out por empresa do Dashboard (~1-8s pra 5-20 empresas numa data). Aceito
  explicitamente pelo usuário [2026-08-10]: "manter migrado, aceitar a 1ª carga
  lenta" — o cache de 1h pra datas passadas faz a MESMA janela recarregar
  instantânea depois da 1ª vez. **`pages/rotinas_diarias.py` tem o formato ainda
  mais caro** (carteiras de TODAS as empresas × grid de um mês inteiro) — é o motivo
  concreto pelo qual a migração geral foi pausada antes de tocar nele; qualquer
  migração futura desse arquivo deve medir a latência real (igual foi feito aqui)
...
- [ ] Toda função **nova ou alterada** tem comentário acima no formato **contexto +
      pseudocódigo**.
...
- [ ] Acesso a dados novo passa por `beehus_api/` quando o endpoint existir; Mongo
      direto só onde já é hoje (leitura) ou como fallback documentado (seção 8).
